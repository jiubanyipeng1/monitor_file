...
更新：
    1. 检查文件是否存在 接口，改为使用列表的方式传入要检查数据，一次最大检查100条，接口以字典返回
修复：
    1. 当开启以用户为文件夹时，文件添加验证许可凭证的接口，并没有以用户文件夹的数据添加到文件路径中
</pre>
版本：version 2.2 client
<pre>
注意事项：
    1. 每天凌晨2类会执行检查一次(暂为启用)
    2. 文件夹的操作（删除、移动等操作） 不会触发服务端的更新，仅操作文件会触发
    3. 文件添加许可的请求使用的是json格式发送，其他都是data
修复：
    由于日志路径设置问题，把它设置到系统盘下了，先已修改为当前路径下
待完成：
    1. 但由于Windows分区的情况，无法进行多个磁盘文件的监视，只能使用多个客户端进行
    2. 上传单个文件使用多线程没有做好并发的准备，暂时关闭
    3. 用户许可过期重新登录
    4. 重新修改文件验证接口，一个个文件进行查询太慢了
更新：
    1. 检查本地文件是否存在服务器函数，更该为多个文件
    2. 客户端请求文件添加许可凭证的获取
    3. 修改文件验证接口，使用列表形式进行查询，列表满足一百进行查询，减少与服务器的交互时间
</pre>
<pre>
版本：2.1 
日期: 2024-08-26
